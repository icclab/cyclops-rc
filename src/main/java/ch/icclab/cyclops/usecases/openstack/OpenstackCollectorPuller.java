 ...
 */
public class OpenstackCollectorPuller {
    final static Logger logger = LogManager.getLogger(OpenstackCollectorPuller.class.getName());
     ...
     */
    private OpenstackCollectorUDRResponse parseResponse(String response) {
        Gson gson = new Gson();

        return gson.fromJson(response, OpenstackCollectorUDRResponse.class);
    }
    ...
    private String constructUsageQuery(String fromDate) {
        if (fromDate.isEmpty()) {
            return udrEndpoint + "/openstackcoll/usage";
        } else {
            return udrEndpoint + "/openstackcoll/usage?from=" + fromDate;
        }
    }
}